...
getVar <- function(crrSPS = c()) {
    if (grepl(paste0("^DATASET|^GET FILE|^GET DATA|^SAVE OUTFILE=|^NEW FILE|^MATCH FILES|^USE ALL$|^FILTER|^EXECUTE$|^TITLE|^COMPUTE|^DELETE VARIABLES|^FORMATS|^VARIABLE LABELS|^VALUE LABELS|",
                     "^SORT CASES|^SPLIT FILE|^SET\\s+|^PRESERVE|^RESTORE|^CACHE|^CTABLES|^GRAPH"), crrSPS[1])) { return(""); }
    ...
    if (any(grepl("VARIABLES", crrSPS))) {
        ...
        vecVar = unlist(strsplit(gsub("\\s*=\\s*", "", strsplit(crrSPS[grepl("VARIABLES", crrSPS)], "VARIABLES\\s?")[[1]][2]), "\\s+"));
    ...
    # afterwards, go through a list with keywords to match and remove 
        ...
        for (begKey in list(c("^GLM\\s+"), c("^UNIANOVA\\s+"), c("METHOD=", "ENTER", "STEPWISE"), c("^MEANS", "TABLES="), c("^SUMMARIZE", "TABLES="), c("^ONESAMPLE", "TEST\\s+\\(", "\\)"),
                            c("^T-TEST\\s+PAIRS=", "\\(PAIRED\\)"), c("COUNT", "^.*?=\\s*", "\\(.*?\\)$"), c("RECODE", "\\(.*?\\)", "INTO.*?$"))) {
