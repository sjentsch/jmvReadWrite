recode <- function(crrCmd = c(), data = data.frame()) {
    # get rid of the RECODE-command in case SPSS-syntax is submitted
#   isSPSS <- grepl("^RECODE", crrCmd)
    crrCmd <- trimws(gsub("\\.$", "", crrCmd))
    rcdSpl <- c(regexpr("\\(", crrCmd)[[1]], max(gregexpr("\\)", crrCmd)[[1]]))
    rcdLst <- strsplit(strSpl(substr(crrCmd, rcdSpl[1] + 1, rcdSpl[2] - 1), "\\)\\s*\\("), "=")
    rcdVrO <- fixVar(strSpl(trimws(gsub("^RECODE", "",  substr(crrCmd, 1, rcdSpl[1] - 1))), "\\s+"), crrCmd, names(data))
    rcdVrT <-        strSpl(trimws(gsub("^\\s*INTO", "", substr(crrCmd, rcdSpl[2] + 1, nchar(crrCmd)))), "\\s+")
    ...
    for (j in seq_along(rcdLst)) {
        ...
        if (grepl("character", class(data[[rcdVrO[1]]]))) {
            ...
            for (k in seq_along(rcdVrO)) {
                chkRcd(class(data[[rcdVrO[k]]]), class(rcdTmp[[k]]), rcdIsC(rcdLst[[j]][1]), rcdIsC(rcdLst[[j]][2]), rcdVrO[k], rcdVrT[k], crrCmd)
...
chkRcd <- function(clsVrO = "", clsVrT = "", isCrpO = FALSE, isCrpT = FALSE, nmeClO = "", nmeClT = "", crrCmd = "") {
