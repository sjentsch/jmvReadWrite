test_that("transpose_omv works", {
    ...
    expect_identical(dim(df4Chk), c(75L, 17L))
    expect_named(df4Chk, c("ID", sprintf("V_%d", seq(16))))
    expect_identical(vapply(df4Chk, typeof, character(1), USE.NAMES = FALSE), c("character", rep("integer", 16)))
    unlink(nmeOut)
    ...
    expect_s3_class(df4Chk, "data.frame")
    expect_identical(dim(df4Chk), c(75L, 17L))
    expect_named(df4Chk, c("ID", sprintf("Qst_%02d", seq(16))))
    expect_identical(vapply(df4Chk, typeof, character(1), USE.NAMES = FALSE), c("character", rep("integer", 16)))
    unlink(nmeOut)
    ...
    expect_s3_class(df4Chk, "data.frame")
    expect_identical(dim(df4Chk), c(75L, 17L))
    expect_named(df4Chk, c("ID", sprintf("Trl_%02d", seq(16))))
    expect_identical(vapply(df4Chk, typeof, character(1), USE.NAMES = FALSE), c("character", rep("integer", 16)))
    unlink(nmeOut)
