...
#' @param varOrd Character vector with the desired order of variable(s) in the data frame (see Details; default: NULL)
#' @param varMve Named list defining to how much a particular variable (name of a list entry) should be moved up (neg.
#'               value of a list entry) or down (pos. value) in the data frame (see Details; default: NULL)
...
#'
#' @details
#' * `varOrd` is a character vector. If not all variables of the original data set are contained in `varOrd`, a warning
#'   is issued but otherwise the list of variables defined in `varOrd` is used (removing variables not contained in
#'   `varOrd`).
#' * `varMve` is a named list. For example would `list(VARNAME = -3)` move the variable `VARNAME` three positions up in
#'   the list of variables (towards the first column), and `list(VARNAME = 3)` would move it three positions down
#'   (towards the last column). If the number of steps the variable is to be moved leads to the position being either
#'   lower than the first or higher than the total number of variables in the data set, an error message is issued.
#'   Please note that the list entries are processed one after another, that is, for a second list entry, you have to
#'   consider how the first list entry may have changed to order of variables.
#' * Using `varOrd` makes more sense for changing the position of several variables, whereas using `varMve` makes more
#'   sense for one variable. If both parameters are given, a warning is issued and `varOrd` takes precedence.
...
#'
#' @seealso
#' `arrange_cols_omv` internally uses the following functions for reading and writing data files in different formats:
...
#'
arrange_cols_omv <- function(dtaInp = NULL, fleOut = "", varOrd = NULL, varMve = list(), psvAnl = FALSE,
...
updOrd <- function(dtaFrm = NULL, varOrd = NULL, varMve = list()) {
    ...
    if (length(varOrd) > 0) {
        ...
        if (!all(names(dtaFrm) %in% varOrd)) {
            warning("The following variable(s) from the original data set are not contained in varOrd: ",
                    paste(setdiff(names(dtaFrm), varOrd), collapse = ", "))
        }
    }
