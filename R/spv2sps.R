...
spv2sps <- function(fleSPV = "") {
    ...
    txtSPS = c();
    logLst = lstZIP[grepl('outputViewer[[:digit:]]+\\.xml', lstZIP)];
    for (logNme in logLst) {
        logTxt <- rawToChar(readBin(logHdl <- file(logFle <- unzip(fleSPV, logNme, junkpaths = TRUE), 'rb'), 'raw', file.info(logFle)$size)); close(logHdl); unlink(logFle); rm('logHdl', 'logFle');
        crrSPS <- gsub('\\s+', ' ', gsub('\\n', ' ', gsub('&nbsp;', ' ', gsub('\ua0', ' ', gsub('&#160;', ' ', trimws(gsub('<.*?>', ' ', strsplit(strsplit(strsplit(strsplit(logTxt, '<container')[[1]][2], '<!\\[CDATA\\[')[[1]][2], '\\]\\]>')[[1]][1], '</head>')[[1]][2])))))));
        txtSPS <- c(txtSPS, unlist(strsplit(crrSPS, '\\. ')));
    }
    ...
    fleSAV = c();
    nteLst = lstZIP[grepl('[[:digit:]]+_lightNotesData\\.bin', lstZIP) | grepl('[[:digit:]]+_notesData\\.bin', lstZIP)];
    for (nteNme in nteLst) {
        ...
        savPos = nteBin[seq(1, length(nteBin) - 4)] == charToRaw('.') & nteBin[seq(2, length(nteBin) - 3)] == charToRaw('s') & nteBin[seq(3, length(nteBin) - 2)] == charToRaw('a') & nteBin[seq(4, length(nteBin) - 1)] == charToRaw('v') & (nteBin[seq(5, length(nteBin))] == '00' | nteBin[seq(5, length(nteBin))] == '01');
        if (any(savPos)) fleSAV = c(fleSAV, gsub('\\\\', '/', rawToChar(nteBin[seq(max(which(nteBin[seq(1, min(which(savPos)))] == '00')) + 1, min(which(savPos)) + 3)])));
    }
