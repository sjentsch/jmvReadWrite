#' Convert data files (CSV, R, other statistics packages) into .omv-files for the statistical spreadsheet 'jamovi'
#' (<https://www.jamovi.org>)
#'
#' @param fleInp Name (including the path, if required) of the data file to be read ("FILENAME.ext"; default: "");
#'               supports CSV and R-files natively, or other file types if "foreign" or "haven" are installed, see
#'               Details below
#' @param fleOut Name (including the path, if required) of the data file to be written ("FILENAME.omv"; default: "");
#'               if empty, the extension of fleInp is replaced with ".omv"
#' @param varSrt Variable(s) that are used to sort the data frame (see Details; if empty, the row order of the input
#'               file is kept; default: NULL)
#' @param usePkg Name of the package: "foreign" or "haven" that shall be used to read SPSS, Stata and SAS files;
#'               "foreign" is the default (it comes with base R), but "haven" is newer and more comprehensive
...
#'
#' @details
#' * In difference to the remaining helper functions, `convert_to_omv` doesn't accept a data frame as input and it
#'   neither does return a data frame if `fleOut` is left empty: If you want to write a data frame, use `write_omv`. If
#'   you want to have a data frame returned use `read_omv` for jamovi-files or any of the functions listed in the
...
#'   reading and writing the data. By clicking on the respective function under “See also”, you can get a more detailed
#'   overview over which parameters each of those functions take. The functions are: `read_omv` and `write_omv` (for
#'   jamovi-files), `read.table` (for CSV / TSV files; using similar defaults as `read.csv` for CSV and `read.delim`
#'   for TSV which both are based upon `read.table`), `load` (for .RData-files), `readRDS` (for .rds-files), `read_sav`
#'   (needs the R-package `haven`) or `read.spss` (needs the R-package `foreign`) for SPSS-files, `read_dta`
#'   (`haven`) / `read.dta` (`foreign`) for Stata-files, `read_sas` (`haven`) for SAS-data-files, and `read_xpt`
#'   (`haven`) / `read.xport` (`foreign`) for SAS-transport-files. If you would like to use `haven`, you may need to
#'   install it using `install.packages("haven", dep = TRUE)`.
#'
#' @seealso `convert_to_omv` internally uses the following functions for reading and writing data files in different
...
#'   [haven::read_sas()] for SAS-data-files, and [haven::read_xpt()] or [foreign::read.xport()] for
#'   SAS-transport-files.
#'
#' @examples
