...
#' @param varPrs Definition of variable pairs; a list containing either list(s) or character vector(s) with the names
#'               of pairs of variables to be combined (default: list()).
#' @param mdeCmb Mode of combining the variables when conflicting values occur, either "none", "first", or "second"
#'               (default: "none"), see Details below.
...
#'
#' @details
#' * The need to combine two columns into one is quite common after merging columns or rows (using, e.g.,
#'   `merge_cols_omv` or `merge_rows_omv`). `varPrs` defines the variable pairs to be combined. It is a list containing
#'   the pairs of variables to be combined, either as list or as character vector; e.g., list(c("A", "B"), c("C", "D"))
#'   or list(list("A", "B"), list("C", "D")). `mdeCmb` defines what to to if values in the first and the second
#'   variable of a variable pair contain conflicting / different values: "none" does not merge the variables (and
#'   instead throws an error), "first" makes that the values from the first variable of each pair are taken if the
#'   values are conflicting, and "second" use the values from the second variable of each pair in case of conflicts.
...
#'
#' @seealso
#' `combine_cols_omv` uses the following functions for reading and writing data files in different formats:
