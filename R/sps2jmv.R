...
sps2jmv <- function(vecSPS = c(), fleSPS = "", fleSAV = "", fltCnd = "") {
    ...
    for (txtSPS in vecSPS) {
        ...
        if (grepl(grpChV, crrSPS[1])) {
            ...
            warning(sprintf("SPSS-command \"%s\" not (yet) implemented.", crrSPS[1]));
            next
        }
...
#           crrArg = updArg(getArg(crrFnc), pairlist());
        ...
        # https://www.ibm.com/docs/en/spss-statistics/SaaS?topic=reference-unianova
            ...
            # effect size, POSTHOC, PLOT-PROFILE and MISSING subcommands
            if (any(grepl('^PRINT|^STATISTICS', crrSPS) & grepl('ETASQ', crrSPS))) { crrArg = updArg(pairlist(effectSize = 'partEta')); }
            if (any(grepl("^POSTHOC", crrSPS)))                                    { crrArg = updArg(crrArg, argPHT(crrSPS, crrFnc)); }
            if (any(grepl("^PLOT.*?PROFILE", crrSPS)))                             { crrArg = updArg(crrArg, argEmm(crrSPS, crrFnc)); }
            if (any(grepl("^MISSING", crrSPS)))                                    { crrArg = updArg(crrArg, argMsV(crrSPS)); }
...
#           crrArg = updArg(getArg(crrFnc), pairlist());
        ...
        # other not (yet) implemented SPSS-commands - remember to include them in incChV in globals.R -------------------------------------------------------------------------------------------------
        # SPSS COMMAND --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        # https://www.ibm.com/docs/en/spss-statistics/SaaS?topic=reference-...
        else if (grepl("^[COMMAND NAME]", crrSPS[1])) {
            stop("[COMMAND NAME] - not implemented yet");
#           crrFnc = "";
#           crrArg = updArg(getArg(crrFnc), pairlist());
...
getVar <- function(crrSPS = c()) {
    ...
    if (any(grepl("VARIABLES", crrSPS))) {
        ...
        vecVar = strsplit(gsub("\\(.*?\\)", "", gsub("\\s*=\\s*", "", gsub("MISSING\\s*=\\s*[A-z]+\\s*", "", gsub("REPORTMISSING\\s*=\\s*NO\\s*", "", strsplit(crrSPS[grepl("VARIABLES", crrSPS)], "VARIABLES\\s?")[[1]][2])))), "\\s+")[[1]];
    ...
    # afterwards, go through a list with keywords to match and remove
        ...
        for (begKey in list(c("COUNT", "^.*?=\\s*", "\\(.*?\\)$"), c("^GLM\\s+"), c("K-S\\(NORMAL\\)="), c("^MEANS", "TABLES="), c("M-W=", "\\(.*?\\)"), c("^ONESAMPLE", "TEST\\s+\\(", "\\)"),
                            c("ONEWAY"), c("RECODE", "\\(.*?\\)", "INTO.*?$"), c("^SUMMARIZE", "TABLES="), c("^T-TEST\\s+PAIRS=", "\\(PAIRED\\)"), c("^UNIANOVA\\s+"), c("WILCOXON=", "\\(PAIRED\\)"))) {
    ...
    chkVar = which(! grepl("^BY$|^WITH$", vecVar));
    for (i in 1:length(chkVar)) {
        if (! any(grepl(paste0("^", vecVar[chkVar[i]], "$"), allVar, ignore.case = TRUE))) {
            if (i < length(chkVar) && any(grepl(paste0("^", vecVar[chkVar[i + 0]], vecVar[chkVar[i + 1]], "$"), allVar))) { vecVar[chkVar[i + 1]] = paste0(vecVar[chkVar[i + 0]], vecVar[chkVar[i + 1]]); vecVar[chkVar[i + 0]] = ""; next }
            if (i > 1              && any(grepl(paste0("^", vecVar[chkVar[i - 1]], vecVar[chkVar[i + 0]], "$"), allVar))) { vecVar[chkVar[i + 0]] = paste0(vecVar[chkVar[i - 1]], vecVar[chkVar[i + 0]]); vecVar[chkVar[i - 1]] = ""; next }
            if (grepl("^ZRE_\\d+$", vecVar[chkVar[i]])) { warning(sprintf("Variable \"%s\" was generated as an output of an earlier analysis and not contained in the data.", vecVar[chkVar[i]])); vecVar[chkVar[i]] = ""; next }
            stop(sprintf("Variable %s is contained in the SPSS-syntax but not in the current data set.", vecVar[chkVar[i]]));
        }
        ...
        if (! any(grepl(paste0("^", vecVar[chkVar[i]], "$"), allVar))) { vecVar[chkVar[i]] = allVar[grepl(paste0("^", vecVar[chkVar[i]], "$"), allVar, ignore.case = TRUE)]; }
    }
...
argEmm <- function(crrSPS = c(), crrFnc = "") {

    lstEmm = pairlist(emMeans      = fmtFrm(strsplit(strsplit(crrSPS[grepl("^PLOT\\s*=", crrSPS)], "\\(|\\)")[[1]][2], "\\s+")[[1]], crrFnc),
                      emmPlots     = TRUE,
...
argPHT <- function(crrSPS = c(), crrFnc = "") {
    ...
    lstPHT = pairlist(postHoc      = as.list(strsplit(effPHT, "\\s+")[[1]]),
...
cnvFrm <- function(crrSPS = c(), crrFnc = '') {
    clcJmv = "";
    clcR   = "";
    ...
    # CONTAINS(item1, item2, item3, …, in1, in2, in3, …): Determines if any of the items appear in in1, in2, in3, .... Note that most of these arguments are optional -- it is possible to simply use <strong>CONTAINS(needle, haystack)</strong>.
