#' Calculates distances (returns symmetric matrix) from a raw data matrix in .omv-files for the statistical spreadsheet 'jamovi' (<https://www.jamovi.org>)
#'
...
#'               returned instead
#' @param varDst Variable (default: c()) containing a character vector with the names of the variables for which distances are to be calculated. See Details
#'               for more information.
#' @param nmeDst Variable (default: "euclidean") containing a character indicating which distance measure is to be calculated calculated. See Details for more
#'               information.
#' @param clmDst Whether the distances shall be calculated between columns (TRUE) or rows (FALSE; default: TRUE). See Details for more information.
...
#'
#' @details
#' * `varDst` must a character vector containing the variables to calculated distances over. If `clmDst` is set to TRUE, distances are calculated between all
#'   possible variable pairs and over subjects / rows in the original data frame. If `clmDst` is set to FALSE, distances are calculated between participants
#'   and over all variables given in `varDst`.
#' * `nmeDst` can be one of the following distance measures: ``
#' * If `clmDst` is set to `TRUE`, the symmetric matrix that is returned has the size V x V (V being the number of variables in varDst; if `mtxSps` is set to
#'   `TRUE`, the size is V - 1 x V - 1, see below); if `clmDst` is set to `FALSE`, the symmetric matrix that is returned has the size R x R (R being the number
...
#'   retained in the lower triangular, the columns range from the first to the variable that is second to the last in `varDst` (or respectively, the columns
#'   contain the first to the second to the last row of the original dataset when `clmDst` is set to `FALSE`), and the rows contain the second to the last
#'   variable in `varDst` (or respectively, the rows contain the second to the last row of the original dataset when `clmDst` is set to `FALSE`).
...
#'
#' @seealso `distances_omv` internally uses the following functions for reading and writing data files in different formats: [jmvReadWrite::read_omv()] and
...
#'
distances_omv <- function(dtaInp = NULL, fleOut = "", varDst = c(), nmeDst = c("euclidean"), clmDst = TRUE, mtxSps = FALSE, mtxTrL = FALSE, mtxDgn = TRUE,
