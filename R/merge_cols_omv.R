#' Merges two or more data files by adding the content of other input files as columns to the first
...
#'         sets (given in the `dtaInp`-argument) are concatenated
#'
#' @details
#' * Using data frames with the input parameter `dtaInp` is primarily thought to be used when
#'   calling `merge_cols_omv` from the jamovi-modules `jTransform` and `Rj`. For the use in R, it
#'   is strongly recommended to use a character vector with the file names instead.
#' * There are four different types of merging operations (defined via `typMrg`): "outer" keeps all
#'   cases (but columns in the resulting data set may contain empty cells / missing values if same
#'   input data sets did not have a row containing the matching variable (defined in `varBy`).
#'   "inner" keeps only those cases where all datasets contain the same value in the matching
#'   variable, for "left" all cases from the first data set in `dtaInp` are kept (whereas cases
#'   that are only contained in the second or any later input data set are dropped), for "right"
#'   all cases from the second (or any higher) data set in `dtaInp` are kept. The behaviour of
#'   "left" and "right" may be somewhat difficult to predict in case of merging several data sets,
#'   therefore "outer" might be a safer choice if several data sets are merged.
#' * The variable that is used for matching (`varBy`) can either be a string (if all datasets
...
#'   length as it is necessary to always use the same number of matching variables when merging).
#' * `varSrt` can be either a character or a character vector (with one or more variables
...
#'   and [jmvReadWrite::write_omv()] for jamovi-files, [utils::read.table()] for CSV / TSV files,
...
merge_cols_omv <- function(dtaInp = NULL, fleOut = "", typMrg = c("outer", "inner", "left", "right"), varBy = list(),
    ...
    for (i in setdiff(seq_along(dtaFrm), 1)) {
        tmpMrg <- do.call(merge, c(list(x = tmpMrg, y = dtaFrm[[i]], by.x = varBy[[1]], by.y = varBy[[i]]),
                                   crrArg[!grepl("^x$|^y$|^by.x$|^by.y$", names(crrArg))]))
