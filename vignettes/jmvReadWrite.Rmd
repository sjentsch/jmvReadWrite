...
* [`aggregate_omv`](https://sjentsch.github.io/jmvReadWrite/reference/aggregate_omv.html):
  Aggregates data (calculating, e.g., mean, median, etc.) from data sets in long
  format.
...
* [`label_vars_omv`](https://sjentsch.github.io/jmvReadWrite/reference/label_vars_omv.html):
  Labels columns / variables in jamovi data files, using either a CSV file, a data frame (each
  containing two columns, one with the variable names, the other with the variable labels), or
  (3) a character vector with the variable labels.
