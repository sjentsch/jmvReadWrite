test_that("globals work", {
    ...
    expect_identical(adjArg("read_omv", list(fleInp = "Trial_dflArg.omv", getSyn = TRUE, getHTM = TRUE),
    ...
    expect_length(tmpPB, 1)
    expect_length(tmpPB$c$options[[1]]$c$options, 9)
    expect_identical(tmpPB$as.character(),
                     paste0("c {\n  options {\n    c {\n      options {\n        o: NONE\n      }\n      options {\n",
                            "        o: TRUE\n      }\n      options {\n        i: 1\n      }\n      options {\n",
                            "        d: 0.01\n      }\n      options {\n        s: \"Trial\"\n      }\n      options {\n",
                            "        c {\n          options {\n            o: TRUE\n          }\n          options {\n",
    ...
    expect_error(setAtt(attLst = "Trial", inpObj = data.frame(), outObj = data.frame()),
      regexp = "^Error when storing or accessing meta-data information\\. Please send the file")
    expect_error(setAtt(attLst = NULL,    inpObj = data.frame(), outObj = data.frame()),
                 "^setAtt: The parameter attLst is supposed to be a character vector\\.")
    expect_error(setAtt(attLst = "Trial", inpObj = NULL,         outObj = data.frame()),
                 "^setAtt: The parameter inpObj is supposed to be either a list or a data frame\\.")
    expect_error(setAtt(attLst = "Trial", inpObj = data.frame(), outObj = NULL),
                 "^setAtt: The parameter outObj is supposed to be either a list or a data frame\\.")
    expect_named(attributes(rmvAtt(jmvReadWrite::AlbumSales)), c("names", "class", "row.names"))
    expect_identical(setdiff(names(attributes(jmvReadWrite::AlbumSales)), names(attributes(rmvAtt(jmvReadWrite::AlbumSales)))),
                     c("datalabel", "var.labels"))
    unlink(nmeOMV)

    expect_identical(bckAtt(jmvReadWrite::AlbumSales),
      list(dtaFrm = attributes(jmvReadWrite::AlbumSales),
           dtaCol = setNames(rep(list(NULL), ncol(jmvReadWrite::AlbumSales)), names(jmvReadWrite::AlbumSales))))
    expect_identical(bckAtt(NULL), list(dtaFrm = list(), dtaCol = list()))

    tmpDF <- nllAtt(jmvReadWrite::AlbumSales, c("datalabel", "var.labels"))
    expect_named(attributes(tmpDF), names(attributes(jmvReadWrite::AlbumSales)))
    expect_identical(attr(tmpDF, "datalabel"),  character(0))
    expect_identical(attr(tmpDF, "var.labels"), character(0))
    expect_length(attr(tmpDF, "datalabel"),  0)
    expect_length(attr(tmpDF, "var.labels"), 0)

    expect_error(inp2DF(dtaInp = 1),
                 "^dtaInp must either be a data frame or a character \\(pointing to a location where the input file can be found\\)\\.")
    inpDF <- jmvReadWrite::AlbumSales
    inpNme <- file.path("..", "ToothGrowth.omv")
    expect_s3_class(inp2DF(dtaInp = inpNme), "data.frame")
    expect_identical(dim(inp2DF(dtaInp = inpNme)), c(60L, 16L))
    ...
    expect_identical(dim(df4Chk[[1]]), c(200L, 5L))
    expect_identical(dim(df4Chk[[2]]), c(60L, 16L))
    expect_named(df4Chk[[1]], c("selSbj", "Adverts", "Airplay", "Image", "Sales"))
    expect_named(df4Chk[[2]],
                 c("Filter 1", "ID", "logLen", "supp - Transform 1", "len", "supp", "dose", "dose2", "dose3", "Trial",
                   "Residuals", "J", "K", "L", "M", "weights"))
    expect_named(attributes(df4Chk[[1]]), c("datalabel", "names", "var.labels", "class", "row.names", "fleInp"))
    expect_identical(attr(df4Chk[[1]], "fleInp"), file.path("..", "ToothGrowth.omv"))
    ...
    expect_named(df4Chk, c("selSbj", "Adverts", "Airplay", "Image", "Sales"))
    expect_named(attributes(df4Chk), c("names", "row.names", "class"))
    expect_true(all(df4Chk == jmvReadWrite::AlbumSales))

    inpDF <- rbind(jmvReadWrite::AlbumSales[seq(sample(10)), ] * NA, jmvReadWrite::AlbumSales[seq(1, 100), ],
                   jmvReadWrite::AlbumSales[1, ] * NA, jmvReadWrite::AlbumSales[seq(101, 200), ],
                   jmvReadWrite::AlbumSales[seq(sample(10)), ] * NA)
    expect_error(df4Chk <- inp2DF(dtaInp = inpDF, rmvEmp = TRUE),
                 paste("Empty rows are not permitted execpt from the begin or the end of an input data frame \\(in such",
