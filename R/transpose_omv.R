...
#'
#' @details
#' * If `varNme` empty, the row names of the input data set are used (preceded by "V_" if all row names are numbers);
#'   if `varNme` has length 1, then it is supposed to point to a variable in the input data frame; if `varNme` has the
#'   same length as the number of rows in the input data frame, then the values in `varNme` are assigned as column
...
#'
#' @seealso
#' `transpose_omv` internally uses the following functions for reading and writing data files in different formats:
