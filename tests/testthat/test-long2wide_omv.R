test_that("long2wide_omv works", {
    ...
    expect_equal(unname(unlist(sapply(df4Chk, attr, "jmv-desc"))), sprintf("%s (Month: %s)", rep(c("Variable X", "Variable Y"), times = 12), month.abb[rep(1:12, each = 2)]))
    unlink(nmeOut)
    ...
    expect_equal(unname(unlist(sapply(df4Chk, attr, "jmv-desc"))), sprintf("Variable X (Month: %s)", month.abb[1:12]))
    unlink(nmeOut)
    ...
    expect_equal(unname(unlist(sapply(df4Chk, attr, "jmv-desc"))), sprintf("Variable Y (Month: %s)", month.abb[1:12]))
    unlink(nmeOut)
    ...
    expect_equal(unname(unlist(sapply(df4Chk, attr, "jmv-desc"))), c(unname(unlist(lblTmp[1:2])), sprintf("%s (cond: %s, colour: %s, rep: %d)",
    ...
    expect_equal(unname(unlist(sapply(df4Chk, attr, "jmv-desc"))), c(unname(unlist(lblTmp[1])), sprintf("%s (cond: %s)",
      rep(unname(unlist(lblTmp[6:7])), each = 3), rep(c("cong", "incong", "neutral"), times = 2))))
