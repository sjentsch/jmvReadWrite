test_that("read_omv works", {
    ...
    # read the data set (without default arguments) and test its properties: data frame, size, correct column type,
    # attributes (data frame and fourth column), whether the "missingValues"-attribute is an empty list, and whether the
    # factor levels at columns 4, 6, and 8 have the correct length and content
    ...
    expect_identical(vapply(df4Chk, typeof, character(1), USE.NAMES = FALSE),
      c("logical", "character", "double", "integer", "double", "integer", "double", "integer", "integer", "integer",
        "double", "double", "double", "integer", "logical", "integer"))
    expect_named(attributes(df4Chk), c("names", "row.names", "class", "fltLst"))
    expect_named(attributes(df4Chk[[2]]),  c("jmv-id", "missingValues"))
    expect_named(attributes(df4Chk[[4]]),  c("levels", "class", "values", "missingValues"))
    expect_named(attributes(df4Chk[[9]]),  c("levels", "class", "values", "missingValues"))
    expect_named(attributes(df4Chk[[10]]), c("levels", "class", "jmv-desc", "missingValues"))
    expect_vector(attr(df4Chk[[4]],  "missingValues"), list(), 0)
    expect_vector(attr(df4Chk[[14]], "missingValues"), list(), 2)
    expect_vector(attr(df4Chk[[4]], "levels"), NULL, 2)
    expect_vector(attr(df4Chk[[6]], "levels"), NULL, 2)
    expect_vector(attr(df4Chk[[8]], "levels"), NULL, 3)
    expect_identical(attr(df4Chk[[4]], "levels"), c("1", "2"))
    expect_identical(attr(df4Chk[[6]], "levels"), c("OJ", "VC"))
    expect_identical(attr(df4Chk[[8]], "levels"), c("0.5", "1.0", "2.0"))
    expect_identical(attr(df4Chk[[14]], "missingValues"), list("== 99", "== 88"))

    # read the data set (with the useFlt-argument set TRUE) and test its properties: data frame, size, correct column
    # type (NB: two rows and the first column - "Filter 1" are removed)
    df4Chk <- read_omv(fleInp = nmeInp, useFlt = TRUE, rmMsVl = FALSE, sveAtt = FALSE, getSyn = FALSE, getHTM = FALSE)
    expect_s3_class(df4Chk, "data.frame")
    expect_identical(dim(df4Chk), c(58L, 15L))
    expect_identical(vapply(df4Chk, typeof, character(1), USE.NAMES = FALSE),
                     c("character", "double", "integer", "double", "integer", "double", "integer", "integer", "integer",
                       "double", "double", "double", "integer", "logical", "integer"))
    ...
    # read the data set (with the sveAtt-argument set TRUE) and test its properties: data frame, size, correct column
    # type, and several attributes of the whole data frame and columns within it
    ...
    expect_identical(vapply(df4Chk, typeof, character(1), USE.NAMES = FALSE),
                     c("logical", "character", "double", "integer", "double", "integer", "double", "integer", "integer",
                       "integer", "double", "double", "double", "integer", "logical", "integer"))
    expect_named(attributes(df4Chk), c("names", "row.names", "class", "fltLst", "removedRows", "addedRows", "transforms"))
    expect_named(attributes(df4Chk[[4]]),
                 c("levels", "class", "values", "name", "id", "columnType", "dataType", "measureType", "formula",
                   "formulaMessage", "parentId", "width", "type", "importName", "description", "transform", "edits",
                   "missingValues", "trimLevels"))
    expect_named(attributes(df4Chk[[9]]),
                 c("levels", "class", "values", "name", "id", "columnType", "dataType", "measureType", "formula",
                   "formulaMessage", "parentId", "width", "type", "importName", "description", "transform", "edits",
                   "missingValues", "trimLevels"))
    expect_named(attributes(df4Chk[[11]]),
                 c("jmv-desc", "name", "id", "columnType", "dataType", "measureType", "formula", "formulaMessage",
                   "parentId", "width", "type", "outputAnalysisId", "outputOptionName", "outputName",
                   "outputDesiredColumnName", "outputAssignedColumnName", "importName", "description", "transform",
                   "edits", "missingValues"))

    # read the data set (with the getSyn-argument set TRUE) and test its properties: correct attributes (should contain
    # "syntax" and "protobuf"), as well as the type, size and content of "syntax" (should start with "jmv::")
    df4Chk <- read_omv(fleInp = nmeInp, useFlt = FALSE, rmMsVl = FALSE, sveAtt = FALSE, getSyn = TRUE, getHTM = FALSE)
    expect_named(attributes(df4Chk), c("names", "row.names", "class", "fltLst", "syntax", "protobuf"))
    # the next two command actually work in both cases: when a list with "syntax" is filled with command and if it's empty
    expect_vector(attr(df4Chk, "syntax"), NULL, 2)
    ...
    # read the data set (with the getSyn-argument set TRUE) and test its properties: correct attributes (should contain
    # "syntax" and "protobuf"), as well as the type, size and content of "syntax"
    df4Chk <- read_omv(fleInp = nmeInp, useFlt = FALSE, rmMsVl = FALSE, sveAtt = FALSE, getSyn = FALSE, getHTM = TRUE)
    expect_named(attributes(df4Chk), c("names", "row.names", "class", "fltLst", "HTML"))
    expect_vector(attr(df4Chk, "HTML"), character(), 257)
    expect_identical(attr(df4Chk, "HTML")[1],   "<!DOCTYPE html>")
    expect_identical(attr(df4Chk, "HTML")[2],   "<html>")
    expect_identical(attr(df4Chk, "HTML")[257], "</html>")
    expect_length(grep("<.*?>", attr(df4Chk, "HTML")), 72)
    ...
    expect_type(cl4Chk, "logical")
    expect_null(attributes(cl4Chk))
    expect_identical(c(mean(cl4Chk), sd(cl4Chk)), c(mean(tmpCol), sd(tmpCol)))
...
test_that("read_all works", {
    # read_all should work as read_omv (with the sveAtt-argument set TRUE) and have the same properties: data frame,
    # size, correct column type, and several attributes of the data frame and columns
    nmeInp <- file.path("..", "ToothGrowth.omv")
    ...
    # read_all for ToothGrowth as Rdata-set has fewer columns and attributes, check data frame, size, correct column
    # type, and several attributes of the data frame and columns
    nmeTmp <- tempfile(fileext = ".rds")
    saveRDS(jmvReadWrite::ToothGrowth, nmeTmp)
    df4Chk <- read_all(nmeTmp)
    expect_identical(dim(df4Chk), c(60L, 7L))
    expect_identical(vapply(df4Chk, typeof,                  character(1), USE.NAMES = FALSE),
                     c("character", "integer", "integer", "double", "integer", "double", "double"))
    expect_identical(vapply(df4Chk, function(x) class(x)[1], character(1), USE.NAMES = FALSE),
                     c("character", "factor", "factor", "numeric", "ordered", "numeric", "numeric"))
    expect_named(attributes(df4Chk), c("names", "row.names", "class"))
    expect_named(attributes(df4Chk[[3]]),  c("levels", "class", "description"))
    expect_named(attributes(df4Chk[[5]]),  c("levels", "class"))
    unlink(nmeTmp)
    ...
    save(list = grep("D[1-2]", ls(), value = TRUE), file = nmeInp)
    ...
    expect_identical(dim(df4Chk), c(100L, 1L))
    unlink(nmeInp)
    ...
    if (requireNamespace("haven", quietly = TRUE)) {
        ...
        expect_identical(attributes(df4Chk)[c("names", "row.names", "class")],
                     list(names = c("ID", "supp", "supp2", "dose", "dose2", "len", "logLen"), row.names = seq(60), class = "data.frame"))
        expect_identical(lapply(df4Chk, attributes),
    ...
    expect_identical(attributes(clnTbb(dtaTmp, jmvLbl = TRUE)[[6]]), list(`jmv-desc` = "Trial for label conversion"))
    ...
    expect_identical(attributes(clnTbb(dtaTmp)), list(names = "value", row.names = seq(200), class = "data.frame"))
    expect_identical(attributes(clnTbb(dtaTmp)[, "value"]), list(levels = sprintf("Level %d", seq(3)), class = "factor", format.sas = "LEVELS"))
    expect_identical(attributes(clnTbb(dtaTmp, rmvAtt = "format.sas")[, "value"]), list(levels = sprintf("Level %d", seq(3)), class = "factor"))
    expect_identical(as.integer(table(clnTbb(dtaTmp)[, "value"])), c(71L, 69L, 60L))
    expect_named(table(clnTbb(dtaTmp)[, "value"]), sprintf("Level %d", seq(3)))
    ...
    expect_identical(attributes(clnTbb(dtaTmp)), list(names = "value", row.names = seq(200), class = "data.frame"))
    expect_identical(attributes(clnTbb(dtaTmp)[, "value"]), list(format.sas = "LEVELS"))
    expect_null(attributes(clnTbb(dtaTmp, rmvAtt = "format.sas")[, "value"]))
    expect_identical(as.integer(clnTbb(dtaTmp)[, "value"])[1:10], c(4L, 4L, 2L, 2L, 4L, 2L, 4L, 1L, 2L, 4L))
    expect_identical(as.integer(table(clnTbb(dtaTmp)[, "value"])), c(66L, 67L, 67L))
    expect_named(table(clnTbb(dtaTmp)[, "value"]), sprintf("%d", c(1, 2, 4)))
    ...
    expect_identical(attributes(clnTbb(dtaTmp)[, "value"]), list(format.sas = "LEVELS"))
    expect_null(attributes(clnTbb(dtaTmp, rmvAtt = "format.sas")[, "value"]))
    expect_identical(as.integer(clnTbb(dtaTmp)[, "value"])[1:10], c(1L, 1L, 2L, 1L, 1L, 4L, 1L, 2L, 1L, 1L))
    expect_identical(as.integer(table(clnTbb(dtaTmp)[, "value"])), c(70L, 72L, 58L))
    expect_named(table(clnTbb(dtaTmp)[, "value"]), sprintf("%d", c(1, 2, 4)))

    dtaTmp <- structure(list(value = structure(sample(seq(4), 200, replace = TRUE), format.sas = "LEVELS",
                             class = c("haven_labelled", "vctrs_vctr", "double"),
                             labels = setNames(seq(4), c("Smallest", "", "", "Largest")))),
                        class = c("tbl_df", "tbl", "data.frame"), row.names = c(NA, -200L))
    expect_s3_class(clnTbb(dtaTmp), "data.frame")
    expect_s3_class(clnTbb(dtaTmp)[, "value"], "factor")
    expect_identical(attributes(clnTbb(dtaTmp)), list(names = "value", row.names = seq(200), class = "data.frame"))
    expect_identical(attributes(clnTbb(dtaTmp)[, "value"]),
                     list(levels = c("Smallest", "2", "3", "Largest"), class = "factor", format.sas = "LEVELS"))
    expect_identical(attributes(clnTbb(dtaTmp, rmvAtt = "format.sas")[, "value"]),
                     list(levels = c("Smallest", "2", "3", "Largest"), class = "factor"))
    expect_identical(as.integer(clnTbb(dtaTmp)[, "value"])[1:10], c(4L, 2L, 1L, 3L, 2L, 1L, 3L, 3L, 2L, 2L))
    expect_identical(as.integer(table(clnTbb(dtaTmp)[, "value"])), c(52L, 47L, 46L, 55L))
    expect_named(table(clnTbb(dtaTmp)[, "value"]), c("Smallest", "2", "3", "Largest"))
