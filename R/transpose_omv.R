...
#'
#' @details
#' If nmeVar empty, the row names of the input data set are used (preceeded by "V_" if all row names are numbers); if nmeVar has the length 1 then it is supposed to point to a variable in the input
...
transpose_omv <- function(dtaInp = NULL, fleOut = "", nmeVar = "", usePkg = c("foreign", "haven"), selSet = "", ...) {
    ...
    # original data frame are used (preceeded by "V_" if they contain only numbers); if nmeVar has the length 1
    # then it is assumed that it points to 
    if (is.character(nmeVar)) {
        # nmeVar is empty, use row names of the input data frame, if all names are numbers, preceed them with "V_"
