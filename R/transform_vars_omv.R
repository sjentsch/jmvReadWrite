#' Transform skewed variables (aiming at they conform to a normal distribution) in .omv-files for the statistical
#' spreadsheet 'jamovi' (<https://www.jamovi.org>)
#'
#' @inheritParams aggregate_omv dtaInp fleOut usePkg selSet
#' @param varXfm Named list variable where the name indicates which transformation is to be carried out and where each
#'               list entry points to one or more variables to be transformed using this transformation. See Details
...
#'
#' @details
#' * `varXfm` has to be a named list variable where the names can either indicate the type of transformation or the
#'   kind and degree of skewness that shall be corrected. For the type of transformation, the following names are
#'   valid: `posSqr`, `negSqr`, `posLog`, `negLog`, `posInv`, `negInv`; where the second part of the name indicates the
#'   transformation to be carried out: `...Sqr` - square root, `...Log` - logarithm to the basis 10, `...Inv` -
#'   inversion, i.e., 1 / original value), and where the first part of the name indicates whether the original value is
...
#'
#' @seealso
#' `transform_vars_omv` internally uses the following functions for reading and writing data files in different
...
#'
transform_vars_omv <- function(dtaInp = NULL, fleOut = "", varXfm = NULL, psvAnl = FALSE, usePkg = c("foreign", "haven"),
                               selSet = "", ...) {
    ...
    if (!all(unique(unlist(varXfm, use.names = FALSE)) %in% names(dtaFrm))) {
        stop("All columns / variables given in the parameter varXfm need to be contained in the input data frame ",
    ...
    for (crrNme in names(varXfm)) {
        ...
        cmdJmv <- ifelse(crrSfx == "INV",
                         "1 / (RPL_VAR)",
                         sprintf("%s(RPL_VAR)", gsub("SQR", "SQRT", gsub("LOG", "LOG10", crrSfx, fixed = TRUE), fixed = TRUE)))
        for (crrVar in varXfm[[crrNme]]) {
            ...
            tgtAtt <- list(columnType = "Computed", formula = gsub("RPL_VAR", gsub("RPL_VAR", crrVar, rplVar, fixed = TRUE), cmdJmv, fixed = TRUE))
            dtaFrm[, tgtVar] <- eval(parse(text = gsub("SQRT", "sqrt", gsub("LOG10", "log10", gsub("RPL_VAR",
              gsub("RPL_VAR", paste0("dtaFrm[, \"", crrVar, "\"]"), gsub("VMIN", "min", gsub("VMAX", "max", rplVar, fixed = TRUE),
              fixed = TRUE), fixed = TRUE), cmdJmv, fixed = TRUE), fixed = TRUE), fixed = TRUE)))
